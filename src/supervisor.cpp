        ...
        try {
            {
                ...
                for (auto *batch : m_batches) {
                    ...
                    if (num_tasks > 0) {
                        // 不超过 max_workers ，尽可能多的开启运行剩余tasks的线程
                        size_t n_worker_to_add = std::min(m_max_workers - num_workers, num_tasks - num_workers);
                        for (size_t i = 0; i < n_worker_to_add; ++i) {
                            batch->add_worker();
                        }
                    } else if (num_workers > m_min_workers) {   // 没有任务时，保持最低数量 worker
                        batch->del_worker();
                    }
                }
                if (!m_stop) {
                    m_spv_cv.wait_for(lock, std::chrono::milliseconds(m_timeout));
                }
            }
            m_tick_callback();
...
void supervisor::supervise(threadbatch& batch) {
    // lock so that the member variables only can be modified before supervisor::run thread begin 
    // or  supervisor::run thread is waiting for m_spv_cv.
